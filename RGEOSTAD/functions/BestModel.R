BestModel<- function (CoorX, CoorY, Variable, Direccion, Tol, NIntervalos, 
          Lags, Npares, MainTitle) 
{
  ...
  legend(0, MaxY, c(modeloele$cov.model, "Variance", "Sill+Nugget", 
  ...
  colnames(ModMejor) <- c("Nugget", "Sill+Nugget", "Range", 
                          "SCE", "MaxY", "MinY")
  return(ModMejor)
}